...
mod test {
    ...
    #[test]
    fn test_sync_message_callback() {
        let doc1 = Doc::new();
        let txt = doc1.get_or_insert_text("text");
        txt.push(&mut doc1.transact_mut(), "hello world");
        let awareness1 = Awareness::new(doc1);

        let doc2 = Doc::new();
        let _callback = doc2.observe_update_v1(|_, _| {
            println!("执行回调函数");
        });
        let awareness2 = Awareness::new(doc2);

        // 模拟同步过程 - 步骤1: 发送SyncStep1消息
        ...
        let response = protocol
            .handle_message(&awareness2, sync_step1_msg)
            .unwrap();
