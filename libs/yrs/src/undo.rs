...
/// Undo broadcast_manager is a structure used to perform undo/redo operations over the associated shared
/// type(s).
///
...
/// back.
///
/// Users can also subscribe to change notifications observed by undo broadcast_manager:
...
///    broadcast_manager as a result of calling either [UndoManager::undo] or [UndoManager::redo] method.
    ...
    M: Meta + 'static,
{
    ...
    /// type and document. While it's possible for undo broadcast_manager to observe multiple shared types
    ...
    /// without any pre-initialize scope. While it's possible for undo broadcast_manager to observe multiple
    ...
    fn should_skip(inner: &Inner<M>, txn: &TransactionMut) -> bool {
        ...
        !inner
            ...
            || !txn
                ...
                .unwrap_or(inner.options.tracked_origins.len() == 1) // tracked origins contain only undo broadcast_manager itself
    }
    ...
    /// Extends a list of shared types tracked by current undo broadcast_manager by a given `scope`.
    ...
    /// Extends a list of origins tracked by current undo broadcast_manager by given `origin`. Origin markers
    ...
    /// Removes an `origin` from the list of origins tracked by a current undo broadcast_manager.
    ...
    /// Returns a list of [StackItem]s stored within current undo broadcast_manager responsible for performing
    ...
    /// Undo last action tracked by current undo broadcast_manager. Actions (a.k.a. [StackItem]s) are groups
    ...
    /// Redo'es last action previously undo'ed by current undo broadcast_manager. Actions
...
/// deletions tracked by a corresponding undo broadcast_manager. Whenever an [UndoManger::undo] or
/// [UndoManager::redo] methods are called a last [StackItem] is being used to modify a state of
/// the document.
///
/// Stack items are stored internally by undo broadcast_manager and created automatically whenever a new
...
mod test {
    ...
    fn issue_371() {
        ...
        assert!(!mgr.undo_blocking()); // no more changes tracked by undo broadcast_manager
    }
